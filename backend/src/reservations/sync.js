  ...
  for (const r of reservations) {
    ...
    const result = await query(
      `INSERT INTO guesty_reservations (
         ...
         room_revenue_minor, cleaning_fee_minor, taxes_minor,
         host_payout_minor, host_service_fee_minor, sub_total_minor
       )
       ...
       ON CONFLICT (tenant_id, guesty_id) DO UPDATE SET
         ...
         host_payout_minor      = EXCLUDED.host_payout_minor,
         host_service_fee_minor = EXCLUDED.host_service_fee_minor,
         sub_total_minor        = EXCLUDED.sub_total_minor,
         synced_at              = NOW(),
         updated_at             = NOW()
      ...
      [
        ...
        JSON.stringify(r),
        majorToMinor(financials.roomRevenue),
        majorToMinor(financials.cleaningFee),
        majorToMinor(financials.taxes),
        majorToMinor(financials.hostPayout),
        majorToMinor(financials.hostServiceFee),
        majorToMinor(financials.subTotal),
      ],
    );
