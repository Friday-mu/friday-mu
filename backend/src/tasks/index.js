...
//
// Status set: todo / in_progress / paused / reported /
//             awaiting_approval / completed / cancelled
//   - PATCH to 'completed' sets `completed_at = NOW()`
//   - PATCH out of 'completed' clears it
//   - `done` is accepted as an alias for `completed` (back-compat
//     with the narrow mig-050 enum; auto-mapped at the route layer)
//
...
//
// Email on assignment change is best-effort via tenants/email.js
// `tplTaskAssigned`. Self-assignment is silenced; each newly-added
// assignee gets one email per change.
...
const VALID_STATUS = new Set([
  'todo', 'in_progress', 'paused', 'reported',
  'awaiting_approval', 'completed', 'cancelled',
]);
...
// Mig-050-era callers used 'done'; mig-051 normalises to 'completed'.
...
// Send assignment emails to each newly-added assignee (set
// difference: new minus old). Self-assignment silenced.
  ...
  try {
    ...
    for (const a of assignees) {
      ...
      const tpl = tplTaskAssigned({
        ...
        taskUrl: `https://gms.friday.mu/fad?m=operations&task=${task.id}`,
      });
      sendEmail({ to: a.email, ...tpl }).catch((e) => {
        console.warn(`[tasks/notifyAssignees] sendEmail to ${a.email} failed:`, e.message);
      });
    }
  } catch (e) {
...
router.get('/', attachIdentity, async (req, res) => {
  try {
    ...
    if (req.query.overdue === 'true') {
      ...
      filters.push(`t.status IN ('todo', 'in_progress', 'paused', 'reported')`);
    }
...
router.post('/', attachIdentity, async (req, res) => {
  try {
    ...
    const status = body.status ? normaliseStatus(body.status) : 'todo';
    ...
    await appendActivity(created.id, req.tenantId, {
      kind: 'created',
      actorId: req.identity?.userId || null,
...
router.patch('/:id', attachIdentity, async (req, res) => {
  try {
    ...
    // Activity + email side effects (best-effort).
