...
| PROD-DATA-4 | `frontend/src/app/fad/_data/properties.ts` | Properties + onboarding state + portfolio insights (~30 rows) | `GET /api/properties` (with `?include=onboarding,insights`). **Largely replaced 2026-05-24 (W1):** `useHydratePropertiesFromGuesty` overwrites the `PROPERTIES` fixture at runtime with merged Guesty cache + FAD-native overlay (lifecycle, onboarding checklist, owner contract, tags, amenities, building name). Fixture file kept as initial seed so synchronous consumers don't crash on mount. Demo-only fields still pending W2: portfolio insights / occupancy / ADR / rating. |
...
| PROD-DATA-6 | `frontend/src/app/fad/_data/reservations.ts` | Bookings (~65 rows) | `GET /api/reservations` (with pagination + filters). **Largely replaced 2026-05-24 (W1):** `useLiveReservations` returns merged shape including FAD-native overlay (cleaning_arrangement, special_requests, internal_notes, driver_assignee, planned-vs-actual arrivals, refund state). `RESERVATIONS` fixture still exists as a back-compat shim for unconverted consumers. |
...
| PROD-DATA-41 | `frontend/src/app/fad/_data/reservations.ts:628` `INQUIRIES` | 5 inquiry records with named guests (Elena Rossi, James Thompson, Sofía Mendez, etc.) and quote amounts in EUR | `GET /api/reservations/inquiries` — **endpoint shipped 2026-05-24 (W1)** + `POST /inquiries`, `PATCH /inquiries/:id`, `POST /inquiries/:id/convert`. The `INQUIRIES` constant is now an unused orphan (`InquiriesPage` already consumes `useWebsiteInquiryThreads` from the website-side inbox, not this constant). Safe to delete; left in place pending a follow-up cleanup. The new FAD-side endpoints are ready for the Mathias quote-builder UI (v0.2 §9) when that ships in W2. |
...
| ID | Path | What it does today | Backend action |
|---|---|---|---|
| ~~PROD-LOGIC-1~~ | `frontend/src/app/fad/_components/modules/properties/CreatePropertyDrawer.tsx` | ~~PROPERTIES.push direct fixture mutation~~ | **Closed 2026-05-24 (W1):** `confirmCreate` now `await createProperty(input)` from `propertiesClient.ts` + `hydratePropertiesFromGuesty()` to refetch the fixture from the backend. Submit button shows loading state + handles error toast. |
| ~~PROD-LOGIC-2~~ | `frontend/src/app/fad/_components/modules/reservations/CreateReservationDrawer.tsx` | ~~RESERVATIONS.push direct fixture mutation~~ | **Closed 2026-05-24 (W1):** `confirmCreate` now `await createReservation({ step: 'confirm', … })` from `reservationsClient.ts`. The Draft→Confirm two-step gate from v0.2 §10 is honoured (backend supports both `step: 'draft'` and `step: 'confirm'`). |
| ~~PROD-LOGIC-3~~ | `frontend/src/app/fad/_components/modules/reservations/ReservationDetail.tsx` | ~~Cancel mutated fixture status directly~~ | **Closed 2026-05-24 (W1):** `confirmCancel` now calls `cancelReservation(r.id, reason)` from `reservationsClient.ts`. Backend flips overlay status + writes activity log entry. Phase 2 (write-through to Guesty + owner-notification fire) still pending. |
| PROD-LOGIC-4 | `frontend/src/app/fad/_components/modules/operations/CreateTaskDrawer.tsx` (`parseNl`) | **Offline fallback** for natural-language task creation — regex-based. The primary path is now `POST /api/intent/parse-task` via `_data/intentClient.ts` (Gemini 3.5 Flash primary / Kimi 2.6 fallback). Surfaced as a "Quick draft (offline)" button alongside the smart drafter; will be removed once we're confident in the smart path's uptime. | Delete `parseNl` + the "Quick draft (offline)" button once the AI path's reliability is proven. |
| PROD-LOGIC-5 | `frontend/src/app/fad/_data/pendingCounts.ts` (entire file) | Computes sidebar pending-count badges by filtering local fixtures. Hardcoded `TODAY = '2026-04-27'` baseline. | Suppressed in `liveOnlyMode()` so production shows no fixture-derived sidebar/sub-page urgency. Replace with `GET /api/pending-counts?role=:role&userId=:id` returning role-aware server-computed signals. |
...
| PROD-LOGIC-13 | `frontend/src/app/fad/_data/pendingCounts.ts:217` `pendingFinance` fixture-derived Finance urgency | Finance sidebar/sub-page badges previously read `FIN_EXPENSES` fixture rows and could surface fake operational urgency in production. The old month-end `+1` reconciliation signal was also fixture-derived. | Suppressed in `liveOnlyMode()` so production shows no Finance badge until a real pending-count API exists. When real backend lands, drop the frontend `TODAY_DAY`/fixture scan entirely and rely on server-computed Finance counts. |
