...
| PROD-CONFIG-10 | `app/fad/_components/modules/OperationsModule.tsx` SettingsPage | **Resolved 2026-05-29:** Operations Settings is now editable + persisted per tenant via `GET/PUT /api/operations/settings` (migration 114 `operations_settings` + `src/operations/settings.js` + `_data/operationsSettingsClient.ts`). Task templates / booking-trigger policies / recurring rules are editable with add/remove + live/paused. **Remaining:** the automation JOB that auto-creates tasks from booking-trigger/recurring config is a later slice — config is stored + gated, not yet auto-executing. | (automation-job slice) |
