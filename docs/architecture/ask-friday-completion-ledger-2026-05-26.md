...
Current canonical checkpoint: `origin/fad-rebuild` at `9615b5504eb9acf7ca927784d9df8de8b399de34`; live production reports `28c7794cea388004a9ea16f821177bfe076144e8`.
...
|---|---|---|---|---|---|---|---|---|
| Ask Friday Core | built beyond scaffold | partial global contracts | worker/review/policy scaffold | runtime wired live; PR #59 live adds active runtime draft packs and mirrored learning feedback; PR #62 merged adds reservation/ops planning fixes | focused tests green; live API smoke passed; PR #59 full backend tests green; PR #62 full backend tests green | live at `28c7794c`; PR #62 not deployed | not directly user-facing | deployed control plane, not finished platform |
| FAD Inbox / Friday Consult | mature existing plan | existing Inbox/runtime KB | strong existing harness | learning events + DB turn lease live; PR #59 live seeds draft context pack for `fad_consult` | focused tests green; live structured Consult smoke passed; detail load fix verified; PR #20 Consult draft/timer tests green; PR #27 review-only tests/build green; PR #30 handoff/WhatsApp/draft recovery tests green; PR #35 malformed-envelope tests/build/live smoke green; PR #37 WhatsApp-window schema tests/build/live detail smoke green; PR #39 compact-fallback tests/build/live compose smoke green | live at `28c7794c` | live staff-shaped API smoke passed for review-only and compose drafts; still needs Mary/Franny staff-use proof in browser | Plan 1 code deployed; continue staff-use proof |
| FAD Ops / Friday Consult | strong active plan | strong Ops KB | improving, still young | learning events + schedule constraints live; planner guardrails live; PR #59 live seeds draft context pack for `fad_ops_assistant` and mirrors global Ops turns into that surface; PR #62 merged adds deterministic lunch coverage signal | focused tests green; bounded live Ops prompt smoke passed; PR #21 Ops guardrail tests/build green; PR #25 deterministic fallback tests/build green; PR #33 pricing-cache proof tests/build green; live pricing-caveat smoke passed; 2026-05-29 staff-shaped API smoke passed for schedule, roster, and urgent occupied-property triage; PR #44 compact-first tests/build/live smoke green; PR #59 full backend tests green; PR #62 full backend tests green | live at `28c7794c`; PR #62 not deployed | daily schedule draft, broad schedule-review Consult smoke, and compact-first roster smoke passed; still needs Franny staff-use proof in browser | Plan 1 materially improved; API/model proof passed; staff-use proof still open |
| FAD global Ask Friday | subplan added | module context only | existing FAB/action harness plus TeamInbox/page-focus contract | events + action mirroring live; TeamInbox context and right-panel focus envelope live; Core state/readiness loading live; PR #59 live adds draft context pack and module learning mirrors | focused tests green; live harmless-action smoke passed; PR #41 tests green; PR #42 page-focus contract tests/build green; post-deploy TeamInbox/page-focus API smoke passed; PR #59 full backend tests green | live at `28c7794c` | staff command smoke passed for navigation and page-focus context | Plan 1 mostly clear; frontend can evolve to unified right-panel shell without changing Core ownership |
...
| Website owner enquiry | scoped | public owner skeleton only | existing Website chat | public action route/policy exists; API client action scope deployed | public action request tests green; Website repo not fully useful until separate wiring proves flow | deployed at `af513255` | no confirmed owner-flow usefulness yet | Plan 2, approval-gated actions only |
| Website feedback | scoped | feedback-learning KB shell drafted | existing Website FAB/chat | public action route/policy exists; API client action scope deployed | public action request tests green; Website repo not fully useful until separate wiring proves flow | live at `28c7794c` | no confirmed feedback-learning usefulness yet | Plan 2, approval-gated actions only |
| Guest portal Ask Friday | scoped | guest-portal KB shell drafted | stay-token policy drafted | not wired | no | no | no | later; needs separate authenticated/stay-token route, not public context-pack route |
| Reservations/calendar agent | source-mapped, not full agent | source matrix + KB shell drafted | read-tool/action-request contracts drafted; Core shell readiness live | staff-only read context route live; eval seeds live; KB shell and governed Core shell live; PR #62 merged fixes checkout-excluded overlap and caveats missing calendar coverage; current branch seeds draft context pack | focused tests green locally; live context-tool smoke passed; composer shell smoke passed in PR #48; PR #52 policy/eval/template tests green; PR #62 context-tool tests green | live at `28c7794c`; PR #62 and current branch not deployed | no dedicated agent UI yet | governed Core shell, not a full UI agent |
| Properties agent | source-mapped, not full agent | public/private split + KB shell drafted | read-tool contract drafted; Core shell readiness live | staff-only read context route live; eval seeds live; KB shell and governed Core shell live; current branch seeds draft context pack | focused tests green locally; live context-tool smoke passed; composer shell smoke passed in PR #48; PR #52 policy/eval/template tests green | live at `28c7794c`; current branch not deployed | no dedicated agent UI yet | staff shell governable; public field policy still needs Ishant review before public packs |
| Owners agent | scoped | staff-private positioning + KB shell drafted | lead-capsule and positioning harness drafted; planned shell only | registry/eval seeds live; KB shell live; owner KB alias live but surface remains planned | composer shell smoke passed in PR #48; PR #52 validates planned owner shell draft pack | deployed at `af513255` | no | later until owner-private rules locked |
| Finance agent | scoped | finance KB shell drafted | restricted finance boundary drafted | registry/eval seed only | deterministic privacy seed only | no | no | later until access/redaction locked |
| Legal/admin agent | scoped | legal/admin KB shell drafted | legal-review boundary drafted | registry only | no | no | no | later until legal review rules locked |
| HR/training agent | scoped | HR/training KB shell drafted | HR privacy boundary drafted | registry only | no | no | no | Plan 2/partial; needs restricted HR review lane |
| Analytics/intelligence agent | scoped | analytics-intelligence KB shell drafted | not built | registry only | no | no | no | later after event volume |
| Public MCP | scoped | public-MCP KB shell drafted | scope/action boundary drafted | registry/eval seed only | no | no | no | later; published public packs plus approval-routed requests only |
| Internal agent bridge | scoped | internal-agent bridge KB shell drafted | sanitized-summary/provenance boundary drafted | registry/eval seed only | no | no | no | later; no raw transcript ingestion |
...
- 2026-05-29 live deploy `9097f9bc` included PR #44 compact-first roster consults, PR #46 Plan 2 eval seeds, and commit `9097f9bc` (`feat(public): /api/public/experiences supply hub + Bokun ingestion + channel routing`, including migration `106_experiences.sql`). This deploy was not performed by this Ask Friday Core session. Later deploy `af513255` superseded this checkpoint.
- 2026-05-29 `origin/fad-rebuild` advanced to `b2840671` with commit `12b02389` (`fix(experiences): route Bokun by real country (account is multi-country)`). This follow-up public-experiences routing fix is now included in live `af513255`.
...
- 2026-05-29 PR #52 merged as `b26982c8`, advancing `origin/fad-rebuild` again. It is now included in live `af513255`.
- 2026-05-29 PR #53 merged as `e01945b5`; docs-only ledger sync after PR #52.
- 2026-05-29 PR #54 merged as `0b9c9657`; it grants the Website API client approval-gated public action-request scope. It is now included in live `af513255`.
...
- PR #48 merged on 2026-05-29 as `31fbfd54`; it adds draft KB shells for Reservations/Calendar, Properties, and Owner Enquiry plus composer smoke coverage. It is now deployed in live `af513255`.
- PR #52 merged on 2026-05-29 as `b26982c8`; it adds migration `107_ask_friday_plan2_surface_readiness.sql`. It activates `fad_reservations_calendar_assistant` and `fad_properties_assistant` as governed staff Core shells, keeps `fad_owners_assistant` planned, fixes Plan 2 eval payload action/tool shape, adds deterministic `action_safety` checks, validates context-pack action allowlists against surface policy, requires approval for risky staff shell actions, and adds staff shell draft context-pack templates. It is now deployed in live `af513255`.
- PR #54 merged on 2026-05-29 as `0b9c9657`; it adds migration `108_ask_friday_website_public_action_scope.sql`. It grants `friday-website` the `ask-friday:actions:write` scope needed to enqueue approval-gated public action requests for owner enquiry and feedback surfaces. It is now deployed in live `af513255` and does not bypass surface allowlists or human approval.
- 2026-05-29 live production later advanced to `28c7794c` for both frontend and backend. That deployment includes PR #59 migrations `109`/`110`/`111`, global/module learning feedback, active runtime draft context packs, and the parallel FAD V2 Operations UI pass. PR #62 is merged to origin after that deploy but is not live.
- 2026-05-29 PR #59 merged as `5c1e4b2a`; it adds migrations `109_ask_friday_learning_feedback_policies.sql`, `110_ask_friday_eval_readiness_coverage.sql`, and `111_ask_friday_active_runtime_context_pack_drafts.sql`; mirrors global Ask Friday page-aware learning events into active module surfaces; adds generated active runtime context-pack templates for global Ask Friday, Inbox Consult, and Ops Assistant; seeds non-destructive draft rows for those packs; and extends readiness with learning feedback policy/counts. It is deployed in live `28c7794c`.
- 2026-05-29 PR #62 merged as `9615b550`; it fixes reservation overlap semantics to include check-in and exclude checkout, surfaces missing calendar-cache coverage as caveats, adds deterministic Ops lunch coverage, and tests global Ask Friday module learning-event mirrors. It is not deployed; live remains `28c7794c`.
- Current branch adds migration `112_ask_friday_plan2_context_pack_drafts.sql` so Reservations/Calendar, Properties, and the planned Owners shell have non-destructive draft context packs in the DB instead of only generated templates.
...
Continue Plan 1 browser/workflow usefulness verification on live `28c7794c`, especially real Mary/Franny Inbox and Ops use, while the redesign session stabilizes the UI. In parallel, backend-safe Plan 2 work can continue on context packs, evals, and staff-shell readiness.

As of 2026-05-29, PR #41 was merged into `origin/fad-rebuild` at `61960837` and adds staff-only TeamInbox context for global FAD Ask Friday. PR #42 was merged at `aa7e7f01` and adds the backend page-focus envelope for the shared right-panel Ask Friday design. Both changes are live in deploy `81025039` and remain included in live `28c7794c`.

PR #9 is no longer pending. It was merged as `da67c7be`, and production code now runs `28c7794c`, which includes PR #9, later Feedback FAB evidence-flow work, Website Ask Friday Core scopes, the Inbox/Consult structured-draft bugfix release, the WhatsApp-window Inbox detail fixes, Consult partial-output rejection, bounded Ops Consult compact fallback, the context-pack draft-script env fix, reviewed Website context-pack source tightening, the deterministic Ops Consult fallback, field-staff PWA work, PR #27 review-only Consult suppression, PR #30 Inbox/Consult/Feedback hardening, PR #31 right-panel direction docs, PR #41 TeamInbox context, PR #42 page-focus context, PR #44 compact-first Ops roster consults, PR #46 Plan 2 eval seeds, PR #52 Core shell readiness, PR #54 Website public action scope, PR #57 global Core state/readiness loading, PR #58 learning-event scope repair, and PR #59 active runtime draft/context feedback migrations. Production DB already has the two reviewed Website public context packs published. PR #62 is merged to origin but still not deployed.

Post-PR #62 planning anchors:
...
The next safe action is backend-safe Plan 2 readiness while the redesign session owns Operations/Inbox shell UI: seed missing staff-shell context packs, keep Reservations/Calendar and Properties governed through read tools/action requests, and defer browser/workflow proof until the V2 UI stops blocking realistic testing.
