...
Current canonical checkpoint: `origin/fad-rebuild` at `af513255723d68b3354de351c381f89532366efe`; live production is also `af513255723d68b3354de351c381f89532366efe`. PR #59 is the current undeployed Ask Friday Core delta.
...
|---|---|---|---|---|---|---|---|---|
| Ask Friday Core | built beyond scaffold | partial global contracts | worker/review/policy scaffold | runtime wired live; PR #59 adds active runtime draft packs and mirrored learning feedback | focused tests green; live API smoke passed; PR #59 full backend tests green | deployed code-bearing SHA `af513255`; PR #59 not deployed | not directly user-facing | deployed control plane, not finished platform |
| FAD Inbox / Friday Consult | mature existing plan | existing Inbox/runtime KB | strong existing harness | learning events + DB turn lease live; PR #59 seeds draft context pack for `fad_consult` | focused tests green; live structured Consult smoke passed; detail load fix verified; PR #20 Consult draft/timer tests green; PR #27 review-only tests/build green; PR #30 handoff/WhatsApp/draft recovery tests green; PR #35 malformed-envelope tests/build/live smoke green; PR #37 WhatsApp-window schema tests/build/live detail smoke green; PR #39 compact-fallback tests/build/live compose smoke green | deployed at `af513255`; PR #59 not deployed | live staff-shaped API smoke passed for review-only and compose drafts; still needs Mary/Franny staff-use proof in browser | Plan 1 code deployed; continue staff-use proof |
| FAD Ops / Friday Consult | strong active plan | strong Ops KB | improving, still young | learning events + schedule constraints live; planner guardrails live; PR #59 seeds draft context pack for `fad_ops_assistant` and mirrors global Ops turns into that surface | focused tests green; bounded live Ops prompt smoke passed; PR #21 Ops guardrail tests/build green; PR #25 deterministic fallback tests/build green; PR #33 pricing-cache proof tests/build green; live pricing-caveat smoke passed; 2026-05-29 staff-shaped API smoke passed for schedule, roster, and urgent occupied-property triage; PR #44 compact-first tests/build/live smoke green; PR #59 full backend tests green | deployed at `af513255`; PR #59 not deployed | daily schedule draft, broad schedule-review Consult smoke, and compact-first roster smoke passed; still needs Franny staff-use proof in browser | Plan 1 materially improved; API/model proof passed; staff-use proof still open |
| FAD global Ask Friday | subplan added | module context only | existing FAB/action harness plus TeamInbox/page-focus contract | events + action mirroring live; TeamInbox context and right-panel focus envelope live; Core state/readiness loading live; PR #59 adds draft context pack and module learning mirrors | focused tests green; live harmless-action smoke passed; PR #41 tests green; PR #42 page-focus contract tests/build green; post-deploy TeamInbox/page-focus API smoke passed; PR #59 full backend tests green | deployed at `af513255`; PR #59 not deployed | staff command smoke passed for navigation and page-focus context | Plan 1 mostly clear; frontend can evolve to unified right-panel shell without changing Core ownership |
...
- 2026-05-29 live production now reports `af513255` for both frontend and backend, matching `origin/fad-rebuild`. This means the previously undeployed Core shell and Website public action-scope work is now live along with subsequent Ops/perf fixes. PR #57 added global Ask Friday Core state/readiness loading; PR #58 fixed global Ask Friday learning-event scope drift.
- 2026-05-29 PR #59 is open and clean against `origin/fad-rebuild` at `af513255`, with head `f7157fb9`. It adds migrations `109_ask_friday_learning_feedback_policies.sql`, `110_ask_friday_eval_readiness_coverage.sql`, and `111_ask_friday_active_runtime_context_pack_drafts.sql`; mirrors global Ask Friday page-aware learning events into active module surfaces; adds generated active runtime context-pack templates for global Ask Friday, Inbox Consult, and Ops Assistant; seeds non-destructive draft rows for those packs; and extends readiness with learning feedback policy/counts. It is not deployed.
