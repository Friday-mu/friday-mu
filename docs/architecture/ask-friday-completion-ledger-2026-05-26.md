...
| Reservations/calendar agent | source-mapped, not full agent | source matrix + KB shell drafted | read-tool/action-request contracts drafted; Core shell readiness pending | staff-only read context route live; eval seeds live; KB shell pending deploy; branch `codex/ask-friday-plan2-core-surfaces-20260529` activates governed shell | focused tests green locally; live context-tool smoke passed; composer shell smoke passed in PR #48; branch adds policy/eval/template tests | read tools and eval seeds deployed; PR #48 KB shell and pending 107 shell migration not deployed | no dedicated agent UI yet | read-only staff context is live; pending branch makes it a governed Core shell, still not a full UI agent |
| Properties agent | source-mapped, not full agent | public/private split + KB shell drafted | read-tool contract drafted; Core shell readiness pending | staff-only read context route live; eval seeds live; KB shell pending deploy; branch `codex/ask-friday-plan2-core-surfaces-20260529` activates governed shell | focused tests green locally; live context-tool smoke passed; composer shell smoke passed in PR #48; branch adds policy/eval/template tests | read tools and eval seeds deployed; PR #48 KB shell and pending 107 shell migration not deployed | no dedicated agent UI yet | pending branch makes staff shell governable; public field policy still needs Ishant review before public packs |
| Owners agent | scoped | staff-private positioning + KB shell drafted | lead-capsule and positioning harness drafted; planned shell only | registry/eval seeds live; KB shell pending deploy; branch adds owner KB alias but keeps surface planned | composer shell smoke passed in PR #48; branch validates planned owner shell draft pack | eval seeds deployed; PR #48 KB shell and pending 107 alias update not deployed | no | later until owner-private rules locked |
...
- 2026-05-29 `origin/fad-rebuild` advanced to `7d3a52ba` with PR #50, PR #51, and FAD V2 redesign docs after the live `9097f9bc` deploy. Current pending branch `codex/ask-friday-plan2-core-surfaces-20260529` is rebased on `7d3a52ba`.
