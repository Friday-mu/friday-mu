...
| FAD Ops / Friday Consult | strong active plan | strong Ops KB | improving, still young | learning events + schedule constraints live; planner guardrails live | focused tests green; bounded live Ops prompt smoke passed; PR #21 Ops guardrail tests/build green; PR #25 deterministic fallback tests/build green; PR #33 pricing-cache proof tests/build green; live pricing-caveat smoke passed; 2026-05-29 staff-shaped API smoke passed for schedule, roster, and urgent occupied-property triage; PR #44 compact-first tests/build green | deployed at `9097f9bc` | daily schedule draft and broad schedule-review Consult smoke passed; compact-first roster patch is live but needs same-prompt re-smoke to prove latency/quality | Plan 1 materially improved; roster compact-first live re-smoke and staff-use proof still open |
...
1. Re-smoke the same live roster prompt on `9097f9bc` and confirm response time drops without losing lunch/coverage/fairness checks.
...
Continue Plan 1 browser/workflow usefulness verification on live `9097f9bc`, especially the same Ops roster prompt that previously took about 144s, then return to the broader Ask Friday agent/KB buildout.
