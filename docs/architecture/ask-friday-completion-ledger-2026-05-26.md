...
Current continuation branch: `codex/ask-friday-compact-fallback-ledger-20260529`; live production is `ad2b115146aa26f54d0de1a9069130b1e29a1d5d`.
...
| FAD global Ask Friday | subplan added | module context only | existing FAB/action harness plus TeamInbox/page-focus contract | events + action mirroring live; TeamInbox context merged in PR #41; right-panel focus envelope pending this branch | focused tests green; live harmless-action smoke passed; PR #41 tests green; page-focus contract tests green | deployed at `ad2b1151`; PR #41 merged at `61960837` but not deployed; page-focus branch not deployed | staff command smoke passed for navigation only; TeamInbox/page-focus needs post-deploy smoke | Plan 1 mostly clear; right-panel Core contract now ready for frontend wiring after merge/deploy |
...
4. Pair with Franny/Mary on real use after the `ad2b1151` deploy: the code now blocks known planner failures, the known malformed-envelope leak, the known WhatsApp-window schema error, and the observed compact-fallback compose failure, but staff-use proof is still the team-usefulness gate.
...
As of 2026-05-29, PR #41 was merged into `origin/fad-rebuild` at `61960837` and adds staff-only TeamInbox context for global FAD Ask Friday. It has not been deployed yet. The current follow-on branch adds the backend page-focus envelope for the shared right-panel Ask Friday design; deploy both backend changes together after merge and coordination.
