...
- 2026-05-29 coordinated deploy `81025039` includes PR #41 TeamInbox context, PR #42 right-panel page-focus envelope, the frontend performance/code-split pass, and owner-notification read-side filtering. Frontend/backend live versions align.
...
1. Merge/deploy the roster compact-first patch if review stays green, then re-smoke the same live roster prompt and confirm response time drops without losing lunch/coverage/fairness checks.
2. Continue browser/live workflow QA with real staff-shaped scenarios, especially Mary/Franny use of Inbox compose/review and Franny use of Ops weekly roster/schedule planning.
...
4. Pair with Franny/Mary on real use after the `81025039` deploy: the code now blocks known planner failures, the known malformed-envelope leak, the known WhatsApp-window schema error, the observed compact-fallback compose failure, and the page-focus/TeamInbox context gap, but staff-use proof is still the team-usefulness gate.
...
Continue Plan 1 browser/workflow usefulness verification on live `ad2b1151`, then return to the broader Ask Friday agent/KB buildout.

As of 2026-05-29, PR #41 was merged into `origin/fad-rebuild` at `61960837` and adds staff-only TeamInbox context for global FAD Ask Friday. PR #42 was merged at `aa7e7f01` and adds the backend page-focus envelope for the shared right-panel Ask Friday design. Neither backend change is deployed yet; deploy them together after coordination.
